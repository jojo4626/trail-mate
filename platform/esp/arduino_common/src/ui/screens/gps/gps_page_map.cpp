...
void update_resolution_display()
{
    ...
    double resolution_m = platform::ui::gps::calculate_map_resolution(g_gps_state.zoom_level, map_lat);
...
void tick_loader()
{
    ...
    if (!g_gps_state.initial_tiles_loaded && g_gps_state.map != NULL)
    {
        ...
        g_gps_state.initial_load_ms = sys::millis_now();
    }
    ...
    {
        if (platform::ui::device::sd_ready() && map_source_directory_available(missing_source))
        {
